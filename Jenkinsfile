pipeline {
	...
	stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-16-170-247-234.eu-north-1.compute.amazonaws.com:8080/job/ops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
